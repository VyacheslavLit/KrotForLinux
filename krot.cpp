...
void pauses()
{
	...
	std::cout << std::endl << "Для продолжения нажмите любую клавишу";
	std::cin.get();
	std::cin.clear();
}
...
void Krot::print_map()
{
	...
	for (int i = 0; i < 10; ++i)
	{
		...
		for (int j = 0; j < 10; ++j)
		{
			...
			if (krot_map[j][i] == 9) std::cout << " "<<(char)35;//крот
			if (krot_map[j][i] == 1) std::cout << " " << 'O';//блок
			if (krot_map[j][i] == 2) std::cout << " " << "*";//цель
			if (krot_map[j][i] == 3) std::cout << " " << "0";//занятая цель
		}
		std::cout << "|\n";
	}
	std::cout << "O--------------------O\n";
...
bool Krot::load_map()
{
	...
	in_file >> input;
	count_block = std::atoi(input.c_str());
	//счмтывем координаты блоков
	for (int i = 0; i < count_block; i++)
	{
		in_file >> input;
		int pos_x = std::atoi(input.c_str());
		in_file >> input;
		int pos_y = std::atoi(input.c_str());
	...
	in_file >> input;
	pos_krot.pos_x = std::atoi(input.c_str());
	in_file >> input;
	pos_krot.pos_y = std::atoi(input.c_str());
...
void Krot::go_game()//начало игры
{
	...
	while (true)
	{
		...
		distination = getchar();
		system("stty cooked");
		if (distination == 65) distination_to(0,-1);
		if (distination == 66) distination_to(0,1);
		if (distination == 68) distination_to(-1,0);
		if (distination == 67) distination_to(1,0);
